...
NAV_TITLE_LIST* nav_get_title_list(const char *root, uint32_t flags, void *data)
{
    BD_DIR_H *dir;
    BD_DIRENT ent;
    char *path = NULL;
	...
	if (data == NULL)
		...
		path = str_printf(DIR_SEP "BDMV" DIR_SEP "PLAYLIST");

    dir = dir_open(path, data);
    if (dir == NULL) {
    ...
    for (res = dir_read(dir, &ent); !res; res = dir_read(dir, &ent)) {
		...
		if (data == NULL)
		{
			...
			path = str_printf(DIR_SEP "BDMV" DIR_SEP "PLAYLIST" DIR_SEP "%s", ent.d_name);
		}
...
char* nav_find_main_title(const char *root, void *data)
{
    BD_DIR_H *dir;
    BD_DIRENT ent;
    char *path = NULL;
    ...
    for (res = dir_read(dir, &ent); !res; res = dir_read(dir, &ent)) {
		...
		if (data == NULL)
		{
			...
			path = str_printf(DIR_SEP "BDMV" DIR_SEP "PLAYLIST" DIR_SEP "%s",
					ent.d_name);
		}
...
static void _fill_clip(NAV_TITLE *title,
                       ...
                       NAV_CLIP *clip,
                       unsigned ref, uint32_t *pos, uint32_t *time, void *data)

{
	...
	if (data == NULL)
	{
		...
		path = str_printf(DIR_SEP"BDMV"DIR_SEP"CLIPINF"DIR_SEP"%s.clpi",
				mpls_clip[clip->angle].clip_id);
	}
    clip->cl = clpi_parse(path, 0, data);
    X_FREE(path);
    if (clip->cl == NULL) {
        clip->start_pkt = 0;
        clip->end_pkt = 0;
...
NAV_TITLE* nav_title_open(const char *root, const char *playlist, void *data)
{
    ...
    title->angle_count = 0;
    title->angle = 0;
    title->pl = mpls_parse(path, 0, data);
    if (title->pl == NULL) {
    ...
    for (ii = 0; ii < title->pl->list_count; ii++) {
        ...
        _fill_clip(title, pi->clip, pi->connection_condition, pi->in_time, pi->out_time, clip, ii, &pos, &time, data);
    }
...
NAV_CLIP* nav_set_angle(NAV_TITLE *title, NAV_CLIP *clip, unsigned angle, void *data)
{
    char *path;
    int ii;
    uint32_t pos = 0;
