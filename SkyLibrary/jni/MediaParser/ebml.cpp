 ...
 */
uint32_t
ebml_read_id (FILE *s, int *length)
{
  int i, len_mask = 0x80;
  uint32_t id;

  for (i=0, id=read_char(s); i<4 && !(id & len_mask); i++)
    len_mask >>= 1;
  if (i >= 4)
 ...
 */
uint64_t
ebml_read_length (FILE *s, int *length)
{
  int i, j, num_ffs = 0, len_mask = 0x80;
  uint64_t len;

  for (i=0, len=read_char (s); i<8 && !(len & len_mask); i++)
    len_mask >>= 1;
  if (i >= 8)
    return EBML_UINT_INVALID;
  j = i+1;
 ...
 */
int64_t
ebml_read_int (FILE *s, uint64_t *length)
{
  int64_t value = 0;
  uint64_t len;
  int l;
 ...
 */
long double
ebml_read_float (FILE *s, uint64_t *length)
{
  long double value;
  uint64_t len;
  int l;
  ...
  switch (len)
    {
    case 4:
        value = av_int2flt(read_dword(s));
        break;

    case 8:
        value = av_int2dbl(read_qword(s));
        break;
 ...
 */
char *
ebml_read_ascii (FILE *s, uint64_t *length)
{
  uint64_t len;
  char *str;
  int l;
  ...
  if (read_data(s, str, len) != (size_t) len)
    {
      free (str);
      return NULL;
    }
 ...
 */
char *
ebml_read_utf8 (FILE *s, uint64_t *length)
{
 ...
 */
int
ebml_read_skip (FILE *s, uint64_t *length)
{
  uint64_t len;
  int l;
 ...
 */
uint32_t
ebml_read_master (FILE *s, uint64_t *length)
{
  uint64_t len;
  uint32_t id;
 ...
 */
char *
ebml_read_header (FILE *s, int *version)
{
