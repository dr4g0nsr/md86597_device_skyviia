...
struct dvd_reader_s {
  /* Basic information. */
  /* 0: directory, 1: UDF iso, 2: ISO9660 iso */
  int isImageFile;
...
static dvd_reader_t *DVDOpenImageFile( const char *location, int have_css )
{
  ...
  if( UDFReadBlocksRaw( dvd, 16, 1, isoidf, 0 ) <= 0 )
  {
	  goto open_img_failed;
  } else {
	  if(memcmp((void *)(isoidf + 1), (void *)"CD001", 5) != 0)
		  goto open_img_failed;
  }
  ...
  if( S_ISBLK( fileinfo.st_mode ) ||
    ...
    /* Resolve any symlinks and get the absolute dir name. */
    {
      if( ( cdir  = open( ".", O_RDONLY ) ) >= 0 ) {
        ...
        new_path = (char *)malloc(PATH_MAX+1);
        if(!new_path) {
          goto DVDOpen_error;
        }
...
int DVDDiscID( dvd_reader_t *dvd, unsigned char *discid )
{
  ...
  for( title = 0; title < 10; title++ ) {
    ...
    if( dvd_file != NULL ) {
      ...
      char *buffer_base = (char *)malloc( file_size + 2048 );
                      ...
                      unsigned char *volsetid, unsigned int volsetid_size )
{
  ...
  buffer_base = (unsigned char *)malloc( DVD_VIDEO_LB_LEN + 2048 );
